...
export const ButtonLink = ({
  ...
  return (
    <PrismicNextLink
      className={clsx(
        "button-cutout group inline-flex items-center mx-4 bg-gradient-to-b from-25% to-75% bg-[length:100%_400%] text-black font-bold transition-[filter,background-position] duration-300 hover:bg-bottom hover:text-black",
        /* Colors */
        color === "orange" && "from-brand-orange to-brand-lime",
        color === "purple" && "from-brand-purple to-brand-lime text-white",
        color === "lime" && "from-brand-lime to-brand-orange",
        /* Sizes */
        size === "sm" && "gap-2.5 py-2 text-base",
        size === "md" && "gap-3 px-1 ~py-2.5/3 text-lg",
        size === "lg" && "~gap-3/4 ~px-1/2 ~py-3/4 ~text-lg/2xl",
        className
      )}
      {...props}
    >
